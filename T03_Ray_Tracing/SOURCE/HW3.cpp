...
// Predefined 4 spheres
int		nSpheres = 0;
vec3	center_world[4];		// In the world coordinate system
float	radius[4];
vec3	m_transmission[4];
...
float i_refractive = 3.0;
float r_refractive = 3.0;
float k;

// Compute tje rotation axis and angle from a to b
//
...
// Compute the reflection vector of l with respect to n
vec3
...
vec3
refract(const vec3& l, const vec3& n, bool out2in)
{
	float theta_i, theta_r;
	vec3 axis;

	if (out2in)
		k = i_refractive / r_refractive;
	else
		k = r_refractive / i_refractive;

	computeRotation(normalize(l), n, theta_i, axis);

	theta_r = asin(k * sin(theta_i));

	return	(k * cos(theta_i) - cos(theta_r)) * n - k * normalize(l);
...
{
	// Find the closest intersection within [ray.po, ray.p1]
	int		iSphere = -1;
...
// Ambiemt, diffuse, specular
vec3
phong(const vec3& n, const vec3& v, const Light& l, const vec3& r, int k)
{
	vec3	I = ambient(l);
	...
	if (lambertian > 0)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			I[i] += m_diffuse[k][i] * lambertian * l.diffuse[i];
	...
	if (iObject != -1)	// Hit an object
	{
		for (int i = 0; i < nLights; i++)
		{
			...
			if (jObject == -1)				// Not shadowed
			{
				...
				vec3	r = normalize(reflect(l[i].p_eye, n));	// Reflection of 1234
		...
		if (depth < DEPTH)
		{
			if (reflection)
			{
				if (out2in)
				{
					...
					p = p + 0.01f * r;
					vec3	pr = p + 1.0E10f * r;	// Point far awary from p along r
					Ray		recursiveRay_r(p, pr);
					vec3	I_R = intensity(recursiveRay_r, l, nLights, depth + 1, iObject);
					p = p - 0.01f * r;
			...
			{
				// Transmision ray
				vec3	t = normalize(refract(normalize(ray.p0 - ray.p1), n, out2in));

				p = p + 0.01f * t;
				vec3	pt = p + 1.0E10f * t;	// Point far awary from p along r
				Ray		recursiveRay_t(p, pt);
				vec3	I_T = intensity(recursiveRay_t, l, nLights, depth + 1, iObject);
				p = p - 0.01f * t;

				for (int i = 0; i < 3; i++)
					I[i] += m_transmission[iObject][i] * I_T[i];
			}
		}
	}
...
rayTracing()
{
	// Viewing matrix
	{
		// The camera faces the nagative z-axis as in OpenGL.
...
#pragma omp master
	...
	light[1].p = vec4(-0.5, 0.5, 0.75, 0);	// Directional light
	light[1].ambient = vec3(1.0, 1.0, 1.0);
	light[1].diffuse = 0.5f * vec3(1.0, 1.0, 1.0);
	light[1].specular = vec3(1.0, 1.0, 1.0);
	nLights++;

	// Specular constant and background intensity
	float	d = 2.0f;
	float	r = 1.414f * d;

	// Predefined 4 spheres
	center_world[nSpheres] = vec3(0, 0, 0);		radius[nSpheres] = 0.7f;
	m_diffuse[nSpheres] = vec3(1.0f, 1.0f, 1.0f);
	m_transmission[nSpheres] = vec3(0.95, 0.95, 0.95);
	nSpheres++;

	center_world[nSpheres] = vec3(0, d / 2.0f, -r);		radius[nSpheres] = 0.5f;
	m_diffuse[nSpheres] = vec3(1.0f, 0, 0);
	m_transmission[nSpheres] = vec3(0, 0, 0);
	nSpheres++;

	center_world[nSpheres] = vec3(0, 0, -r);		radius[nSpheres] = 0.5f;
	m_diffuse[nSpheres] = vec3(0, 1.0f, 0);
	m_transmission[nSpheres] = vec3(0, 0, 0);
	nSpheres++;

	center_world[nSpheres] = vec3(0, -d / 2.0f, -r);		radius[nSpheres] = 0.5f;
	m_diffuse[nSpheres] = vec3(0, 0, 1.0f);
	m_transmission[nSpheres] = vec3(0, 0, 0);
	nSpheres++;
	...
	cout << "Keyboard input: m for texture mapping/direct drawing" << endl;
	cout << "Keyboard input: p for parallel computing on/off" << endl;
	cout << "Keyboard input: c for Employ a different color for each sphere on/off" << endl;
	cout << "Keyboard input: t for translucent on/off" << endl;
	cout << "Keyboard input: r for Reflection on/off" << endl;
	cout << "Keyboard input: [1:9] for ray tracing path" << endl;
}
...
main(int argc, char* argv[])
{
	...
	// To allow byte-aligment for an odd numbered width of the ray-traced image
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_SPACE:	pause = !pause; break;
			...
			// Refractive
		case GLFW_KEY_UP:	if (i_refractive < 100) { i_refractive += 0.1; cout << "i_refractive = " << i_refractive << " r_refractive = " << r_refractive << endl; setRayTracingDepth(DEPTH); } break;

		case GLFW_KEY_DOWN:	if (i_refractive > -100) { i_refractive -= 0.1; cout << "i_refractive = " << i_refractive << " r_refractive = " << r_refractive << endl; setRayTracingDepth(DEPTH); } break;
		...
		case GLFW_KEY_P:	useOpenMP = !useOpenMP;
			...
			// different Color
		case GLFW_KEY_C:	useColor = !useColor;
			if (useColor)	cout << "Using different Color" << endl;
			else            cout << "Non-Using different Color" << endl;
			break;

			// different Color
		case GLFW_KEY_T:	translucent = !translucent;
			setRayTracingDepth(DEPTH);
			if (translucent)	cout << "Transmission On" << endl;
			else            cout << "Transmission Off" << endl;
			break;

			// different Color
		case GLFW_KEY_R:	reflection = !reflection;
			setRayTracingDepth(DEPTH);
			if (reflection)	cout << "Reflection On" << endl;
			else            cout << "Reflection Off" << endl;
			break;
		}
	}
}
