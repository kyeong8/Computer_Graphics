...
render(GLFWwindow* window)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			// Material
			for (int k = 0; k < hit_cnt; k++)
				if (arr[k] == (3 * i + j))		flag = 1;

			if (flag)		setupColoredMaterial(vec3(0, 0, 1));			// Selected
...
findNearestHits(int hits, GLuint selectBuffer[64])
{
	hit_cnt = hits;
	bool	diagnosis = true;
	arr = (int*)malloc(sizeof(int) * hit_cnt);
	...
	for (int i = 0; i < hits; i++)
	{
		...
		arr[i] = (int)selectBuffer[index + 3];

		// Choose the nearest one only
		if (z1 < nearest && selectBuffer[index + 3] < 9) { nearest = z1; name = selectBuffer[index + 3]; }
...
mouseButton(GLFWwindow* window, int button, int action, int mods)
{
	...
	if (action == GLFW_RELEASE && button == GLFW_MOUSE_BUTTON_LEFT)
	{
		...
		picked = selectObjects(window, (x0 + x1) / 2, (y0 + y1) / 2, x0 - x1, y0 - y1);
	}
}
