...
part3()
{
	...
	// Contruct mat3 from an array
	float	B_data[9] = { 1.0, 0.0, 0.0, 2.0, 1.0, 0.0, 3.0, 0.0, 1.0 };
	B = glm::make_mat3(B_data);
	cout << "B = " << to_string(B) << endl;

	// Element indexing: B[colum][row] in GLM.	Be careful!
	cout << "3rd col of B = " << to_string(B[2]) << endl;		// 3rd colum
	cout << "3rd col B = " << to_string(column(B, 2)) << endl;	// Third colum
...
part4()
{
	...
	// Scalar multiplication
	cout << "1.5f*A = " << to_string(1.5f * A) << endl;
