...
animate()
{
	...
	// Periodically chagne the exponent and/or cutoff value of the spot light
...
{
	// vsync should be 0 for precise thme stepping.
	vsync = 0;
...
init()
{
	...
	cout << "Keyboard input: left for increasing shininess coefficient" << endl;
	cout << "Keyboard input: right for decreasing shininess coefficient" << endl;
	...
	cout << "Keyboard input: e for time dependent exponent of a sopt light" << endl;
	cout << "Keyboard input: c for time dependent cutoff of a sopt light" << endl;
...
drawArrow(const vec3& p, bool tailOnly)
{
	...
	if (!tailOnly)
	{
		...
		float	arrowShaftLenghth = 0.2f;
		glColor3f(0, 1, 0);
		drawCylinder(arrowShaftRadius, arrowShaftLenghth, 16, 5);

		// Head cone
		float	arrowheadHeight = 0.09f;
		float	arrowheadRadius = 0.06f;
		glTranslatef(0, 0, arrowShaftLenghth + arrowheadHeight);
		glRotatef(180, 1, 0, 0);
...
render(GLFWwindow* window)
{
	...
	// cout << "shininess coefficient = " << shininess_coefficient << endl;
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_LEFT:		if (shininess_coefficient < 128) shininess_coefficient += 1;	 break;
		case GLFW_KEY_RIGHT:	if (shininess_coefficient > 0) shininess_coefficient -= 1;	 break;
		...
		case GLFW_KEY_C: cutoff = !cutoff;	break;
			...
			// Predefinded material parameters
